...
@router.get("/client_all", status_code=200, response_model=List[OrderOut])
async def get_all_current_client_order(session: Session = Depends(get_db),
    ...
    return order.get_all_client_order(int(Authorize.get_jwt_subject()), s=session)
...
async def get_all_current_walker_order(session: Session = Depends(get_db),
                                       Authorize: AuthJWT = Depends(), auth: HTTPAuthorizationCredentials = Security(security)):
    return order.get_all_walker_order(int(Authorize.get_jwt_subject()), s=session)


@router.post("/", status_code=200, response_model=OrderOut)
async def create_order(order_info: OrderCreate, walker_id: int, session: Session = Depends(get_db),
                       Authorize: AuthJWT = Depends(), auth: HTTPAuthorizationCredentials = Security(security)):
