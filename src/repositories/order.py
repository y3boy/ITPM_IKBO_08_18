...
def create_order(o: OrderCreate, client_user_id: int, walker_user_id: int, s: Session):
...
def get_all_client_order(client_user_id: int, s: Session):
    return s.query(Order).filter(Order.client_id == client_user_id).all()


def get_all_walker_order(walker_user_id: int, s: Session):
    return s.query(Order).filter(Order.walker_id == walker_user_id).all()
